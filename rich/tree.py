...
class Tree(JupyterMixin):
    ...
    def _make_guide(self, index: int, style: Style, options: "ConsoleOptions") -> "Segment":
        ...
        index určuje typ čiary:
          0 = SPACE    — prázdne miesto (za posledným uzlom)
          1 = CONTINUE — zvislá čiara (│) pokračujúca nadol
    ...
    def _yield_node_label(
        ...
        style: Style,
        last: bool,
        renderable_lines: List,
    ) -> "RenderResult":
        """Yieldue riadky labelu uzla spolu s vodiacimi znakmi (prefix)."""
        SPACE, CONTINUE = 0, 1
        ...
        while stack:
            ...
            try:
                ...
                if levels:
                    ...
                    levels[-1] = self._make_guide(FORK, guide_style, options)
            ...
            if node.expanded and node.children:
                levels[-1] = self._make_guide(
                    SPACE if last else CONTINUE, levels[-1].style or null_style, options
                )
                levels.append(
                    self._make_guide(END if len(node.children) == 1 else FORK, guide_style, options)
                )
